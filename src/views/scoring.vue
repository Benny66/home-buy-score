...
<template>
  <div class="scoring-container">
    ...
    <el-card class="card" id="basic-form-card">
      ...
      <el-form :model="formData" :label-width="isSmallScreen() ? '50%': '30%'" class="basic-form">
        <el-row :gutter="20">
    ...
    <el-card class="card">
      ...
      <el-table :data="scoreItems" border style="width: 100%">
        <el-table-column prop="dimension" label="一级维度" :min-width="isSmallScreen() ? '50%': '120px'" />
        <el-table-column prop="weight" label="权重" :min-width="isSmallScreen() ? '40%': '80px'"/>
        <el-table-column prop="subItem" label="二级子项" :min-width="isSmallScreen() ? '80%': '120px'"/>
        <el-table-column prop="criteria" label="评分标准（0-10 分）" :min-width="isSmallScreen() ? '100%': '200px'"/>
        <el-table-column label="我的打分" :min-width="isSmallScreen() ? '60%': '180px'">
          <template #default="scope">
            <template v-if="scope && scope.row">
              <el-input v-model.number="scope.row.score" type="number" min="0" max="10" step="1"
                @blur="handleScoreBlur($event, scope.row)" @input="recalc" />
